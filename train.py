...
class train():
    ...
    def train(self):
        ...
        with tf.name_scope("cost"):
            cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=model.out, labels=model.y))
        # train_step = tf.train.AdamOptimizer(lr).minimize(cost)

        predict_op = tf.argmax(model.out, axis=1, name="predictions")
        with tf.name_scope("accuracy"):
            acc = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(model.y, 1), tf.argmax(model.out, 1)), tf.float32))
        # -------------------------------------------------------------------------------------------#
        ...
        with tf.Session() as sess:
            ...
            # 添加list强制装换
            batches = util.batch_iter(list(zip(self.x_train, self.y_train)), batch_size, n_epochs)
...
# Load data
print("Loading data...")
x_, y_, vocabulary, vocabulary_inv, test_size, label = util.load_data()
f_vocab2id = open('data/vocab2id.pickle','wb')
pickle.dump(vocabulary, f_vocab2id)
f_vocab2id.close()
f_id2vocab = open('data/id2vocab.pickle', 'wb')
pickle.dump(vocabulary_inv, f_id2vocab)
f_id2vocab.close()
f_label2id = open('data/label2id.pickle', 'wb')
pickle.dump(label, f_label2id)
f_label2id.close()
